...
void app::run(std::function<bool(engine2D &, float &)> forward)
{
    ...
    while (m_window.isOpen())
    {
        ...
        m_phys_time = (1.f - m_time_smoothness) * m_raw_phys_time + m_time_smoothness * m_phys_time;

        {
            ...
            m_draw_time = (1.f - m_time_smoothness) * m_raw_draw_time + m_time_smoothness * m_draw_time;
        }
        if (m_sync_dt)
            sync_dt();
    }
...
void app::draw_spring(const glm::vec2 &p1, const glm::vec2 &p2)
{
    draw_spring(p1, p2, m_springs_color);
}
void app::draw_rigid_bar(const glm::vec2 &p1, const glm::vec2 &p2)
{
    draw_rigid_bar(p1, p2, m_rigid_bars_color);
}
...
sf::ConvexShape app::convex_shape_from(const geo::polygon &poly) const
{
    ...
    const glm::vec2 centroid = poly.centroid() * PPX_WORLD_TO_PIXEL, origin = -poly.locals()[0] * PPX_WORLD_TO_PIXEL;
...
void app::recreate_window(const sf::Uint32 style, const glm::vec2 &center, const glm::vec2 &size, const char *name)
{
...
{
    const auto center = m_window.getView().getCenter(), size = m_window.getView().getSize();
    recreate_window(style, glm::vec2(center.x, center.y), glm::vec2(size.x, size.y));
}

void app::control_camera()
{
    if (ImGui::GetIO().WantCaptureKeyboard || sf::Keyboard::isKeyPressed(sf::Keyboard::LControl) ||
...
glm::vec2 app::world_mouse() const
{
    return pixel_mouse() * PPX_PIXEL_TO_WORLD;
}
glm::vec2 app::world_mouse_delta() const
{
    return pixel_mouse_delta() * PPX_PIXEL_TO_WORLD;
}

sf::Uint32 app::style() const
{
    return m_style;
}
void app::style(const sf::Uint32 style)
{
    m_style = style;
}
...
const std::vector<scope<sf::Shape>> &app::shapes() const
{
    return m_shapes;
}
cvw::vector<scope<sf::Shape>> app::shapes()
{
    return m_shapes;
}

sf::Shape &app::operator[](std::size_t index) const
{
...
const sf::Color &app::springs_color() const
{
    return m_springs_color;
}
const sf::Color &app::rigid_bars_color() const
{
    return m_rigid_bars_color;
}

void app::entity_color(const sf::Color &color)
{
    m_entity_color = color;
}
void app::springs_color(const sf::Color &color)
{
    m_springs_color = color;
}
void app::rigid_bars_color(const sf::Color &color)
{
    m_rigid_bars_color = color;
}
...
void app::add_fonts() const
{
    ...
    {
        const std::string &path = entry.path().string(), extension = path.substr(path.find(".") + 1, path.size() - 1);
...
std::uint32_t app::integrations_per_frame() const
{
    return m_integrations_per_frame;
}
void app::integrations_per_frame(std::uint32_t integrations_per_frame)
{
    m_integrations_per_frame = integrations_per_frame;
}
...
void app::paused(const bool paused)
{
    m_paused = paused;
}

std::uint32_t app::framerate() const
{
    return m_framerate;
}
...
void app::sync_timestep(const bool aligned_dt)
{
    m_sync_dt = aligned_dt;
}
...
const sf::Time &app::phys_time() const
{
    return m_phys_time;
}
const sf::Time &app::draw_time() const
{
    return m_draw_time;
}
sf::Time app::delta_time() const
{
    return m_phys_time + m_draw_time;
}

const sf::Time &app::raw_phys_time() const
{
    return m_raw_phys_time;
}
const sf::Time &app::raw_draw_time() const
{
    return m_raw_draw_time;
}
sf::Time app::raw_delta_time() const
{
    return m_raw_phys_time + m_raw_draw_time;
}

float app::time_measure_smoothness() const
{
    return m_time_smoothness;
}
void app::time_measure_smoothness(const float smoothness)
{
    m_time_smoothness = smoothness;
}
